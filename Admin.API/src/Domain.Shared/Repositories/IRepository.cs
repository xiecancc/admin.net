...
/// <para>所有查询方法统一使用 List&lt;Expression&gt; 版本，单表达式场景由调用方包装为列表</para>
/// </remarks>
...
public interface IRepository<TDomain> where TDomain : DomainBase, new() {
    ...
    Task<TDomain?> GetAsync(List<Expression<Func<TDomain, bool>>>? predicates = null, CancellationToken cancellationToken = default);
