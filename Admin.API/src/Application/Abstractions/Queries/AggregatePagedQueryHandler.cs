...
/// <summary>
/// 聚合根分页查询处理器基类
/// <para>用于处理聚合根实体的分页查询操作，支持缓存</para>
/// </summary>
...
public abstract class AggregatePagedQueryHandler<TAggregate, TRepository, TQuery, TQueryDto, TResponseDto>(
    IUnitOfWork unitOfWork,
    TRepository repository,
    IMapper mapper,
    ICacheProvider cacheProvider,
    ILogger<AggregatePagedQueryHandler<TAggregate, TRepository, TQuery, TQueryDto, TResponseDto>> logger)
    : AggregateQueryHandler<TAggregate, TRepository, TQuery, TQueryDto, PagedResponse<TResponseDto>>(unitOfWork, repository, mapper, cacheProvider)
    where TAggregate : AggregateBase, new()
    where TRepository : IAggregateRepository<TAggregate>
    where TQuery : AggregatePagedQuery<TQueryDto, TResponseDto>
    ...
    public override async Task<PagedResponse<TResponseDto>> Handle(TQuery request, CancellationToken cancellationToken) {
        ...
        try {
            ...
            Logger.LogWarning("查询 {AggregateType} 分页被取消，页码: {Page}, 页大小: {Size}", typeof(TAggregate).Name, request.Page, request.Size);
            throw;
        }
        catch (Exception ex) {
            Logger.LogError(ex, "查询 {AggregateType} 分页时发生异常，页码: {Page}, 页大小: {Size}", typeof(TAggregate).Name, request.Page, request.Size);
            throw new InvalidOperationException($"查询 {typeof(TAggregate).Name} 分页时发生异常", ex);
        }
    }
}
