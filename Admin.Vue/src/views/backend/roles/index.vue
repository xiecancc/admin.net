<template>
  <div class="roles-container">
    ...
    <el-row :gutter="20">
          ...
          <el-table :data="filteredRoles" v-loading="loading" style="width: 100%">
            ...
            <el-table-column prop="name" label="角色名称">
              <template #default="{ row }">
                {{ row.name }}
                <el-tag v-if="isBuiltInRole(row.code)" size="small" type="warning" style="margin-left: 8px"
                  >内置</el-tag
                >
              </template>
            </el-table-column>
      ...
      <el-col :span="12">
        <el-card shadow="hover" class="roles-tree-card">
          <template #header>
            <div class="card-header">
              <span>角色树形结构</span>
              <el-button type="primary" size="small" @click="loadRoleTree">
                <el-icon>
                  <Refresh />
                </el-icon>
                刷新
          ...
          <el-tree :data="roleTree" :props="treeProps" node-key="id" default-expand-all highlight-current>
    ...
    <el-dialog v-model="dialogVisible" :title="dialogTitle" width="600px" @close="handleDialogClose">
      <el-form ref="roleFormRef" :model="roleForm" :rules="roleRules" label-width="100px">
        ...
        <el-form-item label="角色名称" prop="name">
          <el-input v-model="roleForm.name" placeholder="请输入角色名称（如：管理员）" />
        </el-form-item>
        <el-form-item label="父角色" prop="parentId">
          <el-select v-model="roleForm.parentId" placeholder="请选择父角色（可选）" clearable style="width: 100%">
            <el-option
...
import { rolesApi, menuPermissionsApi, buttonPermissionsApi, apiPermissionsApi } from "@/api";
import { ROLES, isBuiltInRole } from "@/constants/roles.constant";
import type { RoleListDTO, RoleCreateDTO, RoleUpdateDTO, RoleTreeNodeDTO } from "@/api/nswags/auto";
